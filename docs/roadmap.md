...
Current state (completed locally):

- Argo CD uses an app-of-apps root at `k8s/argocd`
- long-running apps use Kustomize bases under `k8s/apps`
- shared resources use Kustomize bases under `k8s/platform`
- local composition lives under `k8s/environments/local`
- migrations, backups, and drills are separated from autosynced resources
- examples and offline raw operator bundles are explicitly not autosynced
- operator chart versions and install order are pinned in `k8s/operators/helmfile.yaml.gotmpl`
- External Secrets remains the owner of application credentials
- local operator Helm releases are disabled because the current controllers were installed from raw bundles
- Strimzi OCI chart rendering is blocked by the local network and needs an internal mirror or manual download
...
- all Kustomize layers render and pass Kubernetes server dry-run
- Argo CD child Applications are synced
- Helmfile parses for local, dev, staging, and prod
- production Helmfile renders after charts are reachable from the deployment network
- do not enable Helm ownership over an existing raw-installed operator without an operator-specific adoption plan
